...
case $os in
	...
	;;
	'WindowsNT')
	echo 'This script does not support Windows right now! Please file this as a bug.'
	exit 2
	;;
	*)
...
sed -e "s/^serial1=!serial!.*\$/${serial_conf}/" "$conf_file" | \
  sed -e "s/!autogen dir prefix!/$(echo "$wd" | sed -e 's/[\/&]/\\&/g')/" >> "$real_conf_file"
